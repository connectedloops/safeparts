...
def render(commits: list, releases: list, arabic: bool = False) -> str:
    ...
    support = (
        'تصف المدخلات التاريخية الدعم وقت كل تغيير، وليست توصيات تثبيت حالية. '
        '[التثبيت الحالي](https://safeparts.netlify.app/help/ar/build-and-run/) · '
        '[الاسترداد بعد توقف دعم سطح المكتب](https://safeparts.netlify.app/help/ar/desktop/)'
        if arabic else
        'Historical entries describe support at the time of each change, not current installation recommendations. '
        '[Current installation](https://safeparts.netlify.app/help/build-and-run/) · '
        '[Retired-desktop recovery](https://safeparts.netlify.app/help/desktop/)'
    )
    lines = ['<!-- Generated by scripts/dev/changelog.py. Do not edit. -->', '', intro, '', support, '']
